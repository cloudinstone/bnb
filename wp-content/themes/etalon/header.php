...
<html <?php language_attributes(); ?>>
   <head>
	...
	<body <?php body_class(); ?>>
	  ...
	  <?php
		if ( ! isset( $redux_ThemeTek['tek-coming-soon-page'] ) ) {
			$redux_ThemeTek['tek-coming-soon-page'] = '';}
		if ( is_page( $redux_ThemeTek['tek-coming-soon-page'] ) && $redux_ThemeTek['tek-coming-soon-page'] != '' ) {

			?>
	   <div class="maintenance"> <?php } ?>

	  <nav class="navbar navbar-default navbar-fixed-top 
	  <?php
		if ( isset( $redux_ThemeTek['tek-menu-style'] ) ) {
			if ( $redux_ThemeTek['tek-menu-style'] == '2' ) {
				echo esc_html( 'contained' ); }
		}
		?>
		 <?php
			if ( isset( $redux_ThemeTek['tek-menu-behaviour'] ) ) {
				if ( $redux_ThemeTek['tek-menu-behaviour'] == '2' ) {
					echo esc_html( 'fixed-menu' ); }
			}
			?>
 <?php
	if ( isset( $redux_ThemeTek['tek-topbar'] ) ) {
		if ( $redux_ThemeTek['tek-topbar'] == '1' ) {
			echo esc_html( 'with-topbar' ); }
	}
	?>
" >
		...
		<!-- Topbar template -->
		<?php
		if ( ! empty( $redux_ThemeTek['tek-topbar'] ) && $redux_ThemeTek['tek-topbar'] == 1 ) {
		   ...
		   <div class="navbar-header page-scroll">
			...
			<div id="main-menu" class="collapse navbar-collapse  navbar-right">
			   <?php
				wp_nav_menu(
					array(
						'theme_location' => 'keydesign-header-menu',
						'depth'          => 2,
						'container'      => false,
	  ...
	  <header id="header">
		 ...
		 <?php endif; ?>
			...
			<?php
	  } elseif ( ! is_404() && ! is_singular( 'themetek_portfolio' ) ) {
